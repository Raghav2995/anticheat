...
const monitorSection = document.getElementById("monitorSection") as HTMLDivElement;
const studentEmailInput = document.getElementById("studentEmail") as HTMLInputElement;
const studentPasswordInput = document.getElementById("studentPassword") as HTMLInputElement;
const studentLoginBtn = document.getElementById("studentLoginBtn") as HTMLButtonElement;
...
const loadAvailableExams = async (): Promise<void> => {
  ...
  try {
    ...
    let data: Array<{ id: string; name: string; eligibleSemester: string }> = [];
    try {
      data = JSON.parse(rawBody) as Array<{ id: string; name: string; eligibleSemester: string }>;
    } catch {
      throw new Error(
        `Exam API returned non-JSON response (status ${response.status}). Ensure API app is running on http://127.0.0.1:4000`
      );
    }
    if (!response.ok) {
...
void loadAvailableExams().catch((error) => {
  // Initial exam loading is deferred until student login.
});

studentLoginBtn?.addEventListener("click", async () => {
  try {
    if (!studentNameInput.value.trim()) {
      throw new Error("Enter your student name before continuing");
    }
    token = "";
    await loginAsStudent();
    loginSection.classList.add("hidden");
    monitorSection.classList.remove("hidden");
    pushLog("Student login successful");
    await loadAvailableExams();
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
